...
const ManageSupplier = ({ items, columns }) => {
  ...
  label: "Action",
  render: (row) => (
    <div className="flex items-center justify-center gap-2 text-red-500 font-bold cursor-pointer" onClick={() => {
      ...
      <div className="px-15 py-5">
        <div className="mt-15 p-20 rounded-xl shadow-[0_4px_15px_rgba(0,0,0,0.25)] ">
          ...
          <div className="flex justify-end mt-5">
            <TextIconButton
              {...(activeTab === "Register Suppliers"
                ? { text: "Add New Supplier", icon: <GoPlusCircle />, className:"px-6 py-4 bg-[#34C759] text-white rounded-full font-bold text-xl"}
                : { text: "Remove All Supplier", icon: <RiDeleteBin6Line /> ,className:"px-6 py-4 bg-[#DB2D30] text-white rounded-full font-bold text-xl"})}
          ...
          <Modal
            ...
            className={`w-[411px] h-[400px] px-[46px] py-[27px] bg-[#F2F2F7]`}
          >
            {activeTab === "Register Suppliers" && (
              <RegisterSupplierModal onClose={() => setIsModalOpen(false)} />
            )}
            {activeTab === "Release Supplier" && (
             <DeleteSupplierModal onClose={() => setIsModalOpen(false)} selectedSupplier={selectedSupplier} />
            )}
          </Modal>
        </div>
